/**
 * useCTOStream — WebSocket hook for CTO (Creator Transfer Ownership) and fee distribution events.
 * Includes demo mode for sparse events.
 */
...
const DEMO_NAMES = ['MoonCat', 'SolDoge', 'PumpKing', 'DeFiApe', 'GigaChad', 'BonkJr']
const DEMO_SYMBOLS = ['MCAT', 'SDOGE', 'PKING', 'DAPE', 'GIGA', 'BONKJ']
...
export function useCTOStream() {
  ...
  const ingest = useCallback((entry: CTOFeedEntry) => {
    ...
    else { s.totalDistributions++; s.totalDistributedSol += entry.totalSol }
    rateCount.current++
  ...
  useEffect(() => {
    ...
    function scheduleDemoEvent() {
      ...
      const entry: CTOFeedEntry = Math.random() > 0.5
        ? {
            id: `cto-${++idCounter.current}`, kind: 'cto', mint: randomAddr(),
            tokenName: DEMO_NAMES[idx] ?? 'Demo', tokenSymbol: DEMO_SYMBOLS[idx] ?? 'DEMO',
            oldCreator: randomAddr(), newCreator: randomAddr(),
            signature: randomSig(), timestamp: new Date().toISOString(), isNew: true,
          }
        : (() => {
            ...
            return {
              id: `dist-${++idCounter.current}`, kind: 'distribution' as const, mint: randomAddr(),
              tokenName: DEMO_NAMES[idx] ?? 'Demo', tokenSymbol: DEMO_SYMBOLS[idx] ?? 'DEMO',
              shareholders, totalSol: Math.round(totalSol * 1000) / 1000,
              signature: randomSig(), timestamp: new Date().toISOString(), isNew: true,
            }
          })()
      ingest(entry)
...
function parsePumpPortalCTO(msg: Record<string, unknown>, idRef: React.MutableRefObject<number>): CTOFeedEntry | null {
  const txType = (typeof msg.txType === 'string' ? msg.txType : '').toLowerCase()
  if (txType === 'cto' || (msg.oldCreator && msg.newCreator)) {
    return {
      id: `cto-${++idRef.current}`, kind: 'cto',
      mint: (msg.mint as string) || '', tokenName: (msg.name as string) || null,
      tokenSymbol: (msg.symbol as string) || null,
      oldCreator: (msg.oldCreator as string) || '', newCreator: (msg.newCreator as string) || '',
      signature: (msg.signature as string) || '', timestamp: new Date().toISOString(), isNew: true,
    }
  }
  return null
}

function parseRelayCTO(msg: Record<string, unknown>, idRef: React.MutableRefObject<number>): CTOFeedEntry | null {
  ...
  if (msgType === 'distribution') {
    const rawSh = msg.shareholders as Array<{ address: string; amountSol: number }> | undefined
    const shareholders = Array.isArray(rawSh)
      ? rawSh.map(s => ({ address: String(s.address), amountSol: Number(s.amountSol) || 0 }))
      : []
    const totalSol = shareholders.reduce((sum, s) => sum + s.amountSol, 0)
    return {
      id: `dist-${++idRef.current}`, kind: 'distribution',
      mint: (msg.tokenMint as string) || (msg.mint as string) || '',
      tokenName: (msg.name as string) || (msg.tokenName as string) || null,
      tokenSymbol: (msg.symbol as string) || (msg.tokenSymbol as string) || null,
      shareholders, totalSol: Math.round(totalSol * 1000) / 1000,
