...
function getMonacoThemeName() {
  if (typeof document === 'undefined') return 'nanohub-light'
  return document.documentElement.dataset.theme === 'dark' ? 'nanohub-dark' : 'nanohub-light'
}
...
function applyMonacoTheme(monaco: NonNullable<ReturnType<typeof useMonaco>>) {
  ...
  monaco.editor.defineTheme(`nanohub-${isDark ? 'dark' : 'light'}`, {
    base,
    inherit: true,
    rules: [
  ...
  monaco.editor.setTheme(`nanohub-${isDark ? 'dark' : 'light'}`)
}
