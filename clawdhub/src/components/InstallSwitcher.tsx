...
export function InstallSwitcher({ exampleSlug = 'sonoscli' }: InstallSwitcherProps) {
  ...
  const command = useMemo(() => {
    switch (pm) {
      case 'npm':
        return `npx @nanosolana/nanohub@latest install ${exampleSlug}`
      case 'pnpm':
        return `pnpm dlx @nanosolana/nanohub@latest install ${exampleSlug}`
      case 'bun':
        return `bunx @nanosolana/nanohub@latest install ${exampleSlug}`
    }
  }, [exampleSlug, pm])
