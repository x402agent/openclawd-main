...
export const getBySolanaWalletInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db
      .query('users')
      .withIndex('by_solana_wallet', (q) => q.eq('solanaWalletAddress', walletAddress))
      .unique()
  },
})
...
export const linkSolanaWallet = mutation({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(userId, {
      ...
      solanaWalletLinkedAt: Date.now(),
      updatedAt: Date.now(),
    })

    // Also link to nanosolanaUsers if a matching wallet-only record exists
    const walletUser = await ctx.db
      .query('nanosolanaUsers')
      .withIndex('by_wallet', (q) => q.eq('walletAddress', walletAddress))
      .unique()

    if (walletUser && !walletUser.galleryUserId) {
      await ctx.db.patch(walletUser._id, { galleryUserId: userId })
    }
  },
})

export const unlinkSolanaWallet = mutation({
  args: {},
  handler: async (ctx) => {
    const { userId } = await requireUser(ctx)
    await ctx.db.patch(userId, {
      solanaWalletAddress: undefined,
      solanaWalletLinkedAt: undefined,
      updatedAt: Date.now(),
    })
  },
})
