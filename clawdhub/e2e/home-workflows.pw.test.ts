...
test('home install switcher and browse CTA work', async ({ page }) => {
  ...
  await expect(page.getByText('npx @nanosolana/nanohub@latest install sonoscli')).toBeVisible()

  await page.getByRole('tab', { name: 'pnpm' }).click()
  await expect(page.getByText('pnpm dlx @nanosolana/nanohub@latest install sonoscli')).toBeVisible()

  await page.getByRole('tab', { name: 'bun' }).click()
  await expect(page.getByText('bunx @nanosolana/nanohub@latest install sonoscli')).toBeVisible()
