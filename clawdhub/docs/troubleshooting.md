...
- Set `TOGETHER_API_KEY` in the Convex environment (not only locally).
...
- On Railway, confirm the Nitro proxy route is running and `CONVEX_SITE_URL` is present at runtime.
...
- The workflow now validates Convex deployment and produces a Netlify-ready web artifact; frontend hosting can pull from Git or upload that artifact.
