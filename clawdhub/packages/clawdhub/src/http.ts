...
const CURL_META_MARKER = '__NANOHUB_CURL_META__'
...
async function fetchJsonFormViaCurl(url: string, args: FormRequestArgs) {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'nanohub-upload-'))
  try {
...
async function fetchBinaryViaCurl(url: string, token?: string) {
  const tempDir = await mkdtemp(join(tmpdir(), 'nanohub-download-'))
