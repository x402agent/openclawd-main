...
function resolveClawdbotStateDir() {
  const solanaosOverride = process.env.SOLANAOS_STATE_DIR?.trim()
  if (solanaosOverride) return resolveUserPath(solanaosOverride)
  const override = process.env.CLAWDBOT_STATE_DIR?.trim()
  if (override) return resolveUserPath(override)
  const solanaosDir = join(resolveHome(), '.solanaos')
  if (existsSync(solanaosDir)) return solanaosDir
  return join(resolveHome(), '.clawdbot')
}

function resolveClawdbotConfigPath() {
  const solanaosOverride = process.env.SOLANAOS_CONFIG_PATH?.trim()
  if (solanaosOverride) return resolveUserPath(solanaosOverride)
