...
async function startLoopbackAuthServerOrSkip(timeoutMs = 2000) {
  try {
    return await startLoopbackAuthServer({ timeoutMs })
  } catch (error) {
...
describe('browserAuth', () => {
  ...
  it('receives token via loopback server', async () => {
    const server = await startLoopbackAuthServerOrSkip()
    if (!server) return
    const payload = {
      token: 'clh_test',
  ...
  it('times out waiting for login', async () => {
    const server = await startLoopbackAuthServerOrSkip(25)
    if (!server) return
