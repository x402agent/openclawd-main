...
export const AgentConfigSpecSchema = type({
  requiredEnv: 'string[]?',
  stateDirs: 'string[]?',
  example: 'string?',
})
...
export const SkillRequiresSchema = type({
  bins: 'string[]?',
  anyBins: 'string[]?',
  env: 'string[]?',
  config: 'string[]?',
...
export const SkillMetadataSchema = type({
  always: 'boolean?',
  skillKey: 'string?',
  primaryEnv: 'string?',
  emoji: 'string?',
  ...
  config: AgentConfigSpecSchema.optional(),
...
// The _SkillMetadataCheck below will fail to compile if this drifts from the schema.
...
type _SkillMetadataInferred = (typeof SkillMetadataSchema)[inferred]
type _AssertExactKeys<A, B> = [keyof A] extends [keyof B] ? [keyof B] extends [keyof A] ? true : never : never
type _SkillMetadataKeysMatch = _AssertExactKeys<SkillMetadata, _SkillMetadataInferred>
// If this line errors, SkillMetadata is out of sync with SkillMetadataSchema
const _skillMetadataKeysCheck: _SkillMetadataKeysMatch = true
void _skillMetadataKeysCheck
