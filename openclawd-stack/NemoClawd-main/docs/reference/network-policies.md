...
description: "Baseline NemoClawd network policy, filesystem rules, and operator approval flow for the openclawd-stack."
keywords: ["nemoclawd network policy", "sandbox egress control operator approval", "openclawd-stack security"]
topics: ["generative_ai", "ai_agents", "solana"]
tags: ["openclawd", "openshell", "sandboxing", "network_policy", "security", "nemoclawd"]
content:
  type: reference
...
* - `xai`
  - `api.x.ai:443`
  - `/usr/local/bin/Clawd`, `/usr/local/bin/openclaw`
  - All methods
...
When the NemoClawd agent attempts to reach an endpoint not listed in the policy, OpenShell intercepts the request and presents it in the TUI for operator review:
...
This opens a split tmux session with the TUI on the left and the NemoClawd agent on the right.
