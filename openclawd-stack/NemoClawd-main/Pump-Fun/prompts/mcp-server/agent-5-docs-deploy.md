...
## Role
You are a Clawd Opus 4.5 agent responsible for creating comprehensive documentation and preparing the MCP server for deployment. Your work ensures users can install, configure, and use the server effectively.
...
A Model Context Protocol (MCP) server that provides Solana wallet operations to AI assistants like Clawd.
...
## Usage with Clawd Desktop

Add to your Clawd Desktop configuration (`~/Library/Application Support/Clawd/Clawd_desktop_config.json` on macOS):
...
### Clawd Desktop doesn't see the server
- Verify the configuration file path
- Ensure the command is correct
- Check Clawd Desktop logs for errors
...
### 2. Clawd Desktop Configuration Examples

**examples/Clawd-desktop.json**:
```json
{
  "mcpServers": {
    "solana-wallet": {
...
**examples/Clawd-desktop-local.json** (for local development):
```json
{
  "mcpServers": {
    "solana-wallet": {
...
Clawd will use the `generate_keypair` tool and provide you with:
...
Clawd will use the `security_audit` prompt to:
...
Clawd will use the `batch_generate` prompt to:
...
**Clawd:** I'd be happy to help you create a vanity Solana address! What prefix or suffix would you like your address to have?

**User:** I want it to start with "NFT"

**Clawd:** Let me first estimate how long that might take...
...
**Clawd:** *Uses generate_vanity tool*
...
- [ ] Clawd Desktop config examples work
