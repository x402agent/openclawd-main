...
flowchart TB
    ...
    subgraph EXT["External Services"]
        ...
        CREDS["Provider APIs (Clawd, GitHub, GitLab, ...)"]
...
```bash
pip install <package>
openshell sandbox create --remote user@host -- Clawd
```
...
- **Automatic discovery**: The CLI scans the user's local machine for existing credentials (environment variables, configuration files) and offers to upload them to the gateway. Supported providers include Clawd, Codex, OpenCode, OpenAI, Anthropic, NVIDIA, GitHub, GitLab, and others.
...
```bash
openshell sandbox create -- Clawd
```
...
```bash
openshell sandbox create --remote user@hostname -- Clawd
```
