...
configuration (for example `Clawd`, `gitlab`, `github`, `outlook`, `generic`, `nvidia`).
...
- `type`: canonical provider slug (`Clawd`, `gitlab`, `github`, etc.)
...
|---|---|---|
| `Clawd.rs` | `ANTHROPIC_API_KEY`, `CLAUDE_API_KEY` | `~/.Clawd.json`, `~/.Clawd/credentials.json`, `~/.config/Clawd/config.json` |
...
`openshell sandbox create --provider gitlab -- Clawd`
...
5. if interactive and missing, auto-create from existing local state
   (uses `ProviderRegistry::discover_existing()`), trying names like `"Clawd"`,
   `"Clawd-1"`, etc. up to 5 retries for name conflicts,
...
```
CLI: openshell sandbox create -- Clawd
  |
  +-- detect_provider_from_command(["Clawd"]) -> "Clawd"
  +-- ensure_required_providers() -> discovers local ANTHROPIC_API_KEY
  |     +-- Creates provider record "Clawd" on gateway with credentials
  +-- Sets SandboxSpec.providers = ["Clawd"]
  +-- Sends CreateSandboxRequest to gateway
        |
        Gateway: create_sandbox()
          +-- Validates provider "Clawd" exists in store (fail fast)
          +-- Persists Sandbox with spec.providers = ["Clawd"]
                ...
                K8s: pod starts openshell-sandbox binary
                  ...
                  |
                    Sandbox supervisor: run_sandbox()
                      ...
                      |     +-- Gateway resolves: "Clawd" -> credentials -> {ANTHROPIC_API_KEY: "sk-..."}
