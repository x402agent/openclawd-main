...
```yaml
network_policies:
  Clawd_code: # <-- map key (arbitrary identifier)
    name: Clawd_code # <-- human-readable name (used in audit logs)
...
3. **Cmdline match** -- any entry in `exec.cmdline_paths` matches `binary.path` (for script interpreters -- e.g., `/usr/bin/node` runs `/usr/local/bin/Clawd`, the exe is `node` but cmdline contains `Clawd`)
...
4. **Cmdline extraction**: `/proc/{pid}/cmdline` is parsed for absolute paths to capture script names (e.g., when `node` runs `/usr/local/bin/Clawd`).
...
network_policies:
  # L4-only: Clawd can reach Anthropic APIs (no L7 inspection)
  Clawd_code:
    name: Clawd_code
    endpoints:
      - { host: api.anthropic.com, port: 443 }
  ...
  Clawd_code_inspected:
    name: Clawd_code_inspected
    endpoints:
