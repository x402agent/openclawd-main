...
```bash
openshell sandbox create -- Clawd        # Auto-creates Clawd provider
...
Supported types: `Clawd`, `opencode`, `codex`, `generic`, `nvidia`, `gitlab`, `github`, `outlook`.
...
The `--from-existing` flag discovers credentials from local state (e.g., `gh auth` tokens, Clawd config files).
...
```bash
openshell sandbox create --name dev --policy ./initial-policy.yaml -- Clawd
```
...
| Create sandbox with tool | `openshell sandbox create -- Clawd` |
