...
**User**: "Let Clawd talk to api.anthropic.com and statsig.anthropic.com, just let everything through."
...
> "I need Clawd to access Anthropic APIs (just let everything through, no inspection needed), and also access our internal docs API at docs.internal:8080 but only for reading. The docs API is HTTP so no TLS."

### Analysis

- Anthropic API: L4-only (no inspection), standard Clawd binary
...
**User**: "Add sentry.io to the Clawd_code policy in my sandbox-policy.yaml"

**Agent workflow**:

1. Read `sandbox-policy.yaml`
2. Find the `Clawd_code` policy
...
**User**: "Create a new policy file at sandbox-policy-staging.yaml with read-only access to api.github.com and full access to api.anthropic.com for Clawd"
