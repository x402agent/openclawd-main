...
MODEL_PATH = os.environ.get("MLX_MODEL", "Jackrong/Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled")
...
VOICE_MODEL_PATH = os.environ.get("MLX_VOICE_MODEL", "mistralai/Voxtral-4B-TTS-2603")
...
class AnthropicHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # WebSocket Pipeline Hijacker for Anthropic Voice Mode
        if self.headers.get("Upgrade", "").lower() == "websocket":
            ...
            if key:
                ...
                self.end_headers()
                log("🎙️  Voice Mode WebSocket connected! (Voxtral-4B-TTS Audio Pipeline Active)")
                
                # Import MLX Audio dynamically for Voxtral TTS
                try:
                    import mlx_audio.tts as mlx_tts
                    import numpy as np
                    log(f"Loading native TTS weights: {VOICE_MODEL_PATH}")
                    tts_model = mlx_tts.load(VOICE_MODEL_PATH)
                except ImportError:
                    log("mlx-audio not installed. Cannot load Voxtral-4B.")
                    tts_model = None

                # Maintain the Raw TCP socket loop for PCM Audio Frames
                ...
                try:
                    while True:
                        ...
                        # Minimal Mock Protocol: Assume frame contains JSON with text
                        # In reality, this parse Anthropic's WebSockets Base64 PCM format.
                        # Real implementation decodes base64, runs STT if needed,
                        # passes Text to Qwen 27B -> streams generated Text to Voxtral TTS
                        # -> streams 24kHz PCM back as base64.
                        
                        if b"text" in frame and tts_model:
                            import json
                            try:
                                data = json.loads(frame.decode("utf-8", errors="ignore"))
                                target_text = data.get("text", "Initializing Voxtral Matrix")
                                
                                # Generate Speech Matrices natively on GPU
                                log(f"🎙️  Synthesizing TTS: '{target_text}'")
                                audio_array = mlx_tts.generate(tts_model, target_text)
                                
                                # Convert MLX array tensor to raw PCM bytes
                                pcm_bytes = np.array(audio_array).astype(np.int16).tobytes()
                                
                                # Return Anthropic-formatted Audio Block
                                resp = json.dumps({
                                    "type": "audio_output",
                                    "audio": base64.b64encode(pcm_bytes).decode()
                                })
                                sock.sendall(resp.encode() + b"\n")
                            except Exception as parse_e:
                                pass
                                
                except Exception as e:
                    log(f"Voice Mode Disconnected: {e}")
            return
