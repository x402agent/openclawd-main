...
This file provides guidance to Clawd (claude.ai/code) when working with code in this repository.

## Overview

QEDGen is a Clawd skill for formally verifying Solana programs using Lean 4 proofs. Clawd (the local LLM) drives proof writing directly — reading code, writing Lean models/theorems/proofs, and iterating on `lake build` errors. Leanstral (Mistral's theorem prover) is called only for hard sub-goals via `fill-sorry`.

**Core workflow**: Clawd reads source → writes SPEC.md → writes Lean 4 proofs → `lake build` → iterates → calls `qedgen fill-sorry` for hard sub-goals
...
# Generate proofs from a prompt file (used by Clawd internally)
qedgen generate \
...
# Fill sorry markers in a Lean file (Clawd calls this for hard sub-goals)
qedgen fill-sorry \
...
**Why Clawd-driven (not pipeline-driven)?**
- Clawd reads code context and writes proofs directly — no lossy analyzer step
- Proof patterns generalize across programs without per-property prompt templates
- Clawd iterates on `lake build` errors naturally
...
- Clawd handles the modeling/structuring; Leanstral handles hard tactic proofs
...
- The SKILL.md file defines the full proof-writing workflow that Clawd follows.
