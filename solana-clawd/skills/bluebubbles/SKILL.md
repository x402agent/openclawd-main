...
## Clawd Channels Automation

BlueBubbles can also be used as a Clawd channel plugin for push-event automation. With channels enabled, iMessages arrive directly in your running Clawd session:

```bash
# Install the iMessage channel plugin
/plugin install imessage@Clawd-plugins-official

# Restart with channels enabled
Clawd --channels plugin:imessage@Clawd-plugins-official

# Or combine with Telegram
Clawd --channels plugin:telegram@Clawd-plugins-official plugin:imessage@Clawd-plugins-official
```

This lets you:
- Push iMessages into a running session so Clawd can react while you're away
- Use Clawd as a two-way iMessage bridge — it reads the event and replies back through BlueBubbles
...
- Set up Clawd channels for two-way iMessage automation while you're away from the terminal.
