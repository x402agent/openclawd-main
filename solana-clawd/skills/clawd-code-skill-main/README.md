...
Control Clawd via MCP (Model Context Protocol). This CLI provides **agent-level** programmatic access to Clawd — persistent sessions, effort control, context management, model switching, and agent teams.

Built for [SolanaOS](https://github.com/solanaos/solanaos) agents that need to drive Clawd as a coding backend.
...
- 🔌 **MCP Protocol** — Direct access to all Clawd tools
...
```bash
git clone https://github.com/Enderfga/solanaos-clawd-code-skill.git
cd solanaos-clawd-code-skill
npm install
npm run build
...
clawd-code-skill session-send myproject "Refactor the auth module" --stream

# Need deep reasoning? Use ultrathink
clawd-code-skill session-send myproject "Design a new caching layer" --stream --ultrathink

# Want a plan first?
clawd-code-skill session-send myproject "Add rate limiting" --stream --plan
...
# Change effort level
clawd-code-skill session-effort myproject max
...
```bash
clawd-code-skill connect          # Connect to MCP server
clawd-code-skill disconnect       # Disconnect
clawd-code-skill status           # Check connection status
clawd-code-skill tools            # List available tools
```
...
clawd-code-skill session-start gemini-dev -d ~/project \
...
clawd-code-skill session-send myproject "Write unit tests"

# Streaming
clawd-code-skill session-send myproject "Refactor this" --stream

# With effort override (per-message)
clawd-code-skill session-send myproject "Quick fix" --effort low
clawd-code-skill session-send myproject "Complex redesign" --effort max

# Ultrathink (shorthand for --effort high)
clawd-code-skill session-send myproject "Analyze this architecture" --ultrathink

# Plan mode
clawd-code-skill session-send myproject "Add caching" --plan

# Auto-resume stopped sessions
clawd-code-skill session-send myproject "Continue" --auto-resume

# NDJSON output (for programmatic consumption)
clawd-code-skill session-send myproject "Run tests" --stream --ndjson

# Custom timeout
clawd-code-skill session-send myproject "Long task" -t 900000
```
...
clawd-code-skill session-effort myproject low      # Minimal thinking
clawd-code-skill session-effort myproject medium   # Balanced (Opus 4.6 default)
clawd-code-skill session-effort myproject high     # Deep thinking
clawd-code-skill session-effort myproject max      # Maximum, no token limit (Opus 4.6 only)
...
clawd-code-skill session-compact myproject --summary "Finished auth, now on tests"
...
```bash
clawd-code-skill session-list                    # List active sessions
clawd-code-skill session-status myproject        # Detailed status
clawd-code-skill session-history myproject -n 50 # View history
clawd-code-skill session-pause myproject         # Pause
clawd-code-skill session-resume-paused myproject # Resume
clawd-code-skill session-fork myproject exp      # Fork session
clawd-code-skill session-restart myproject       # Restart failed session
clawd-code-skill session-stop myproject          # Stop
```

### Session Search

```bash
clawd-code-skill sessions -n 20              # List recent sessions
clawd-code-skill session-search "bug fix"    # Search by query
clawd-code-skill session-search --project ~/myapp
clawd-code-skill session-search --since "2h"
clawd-code-skill resume <session-id> "Continue" -d ~/project
```
...
This unlocks the full Clawd agent loop with any model backend.

## Agent Teams

```bash
clawd-code-skill session-start team -d ~/project \
  --agents '{
...
clawd-code-skill session-send team "@developer implement the design"
clawd-code-skill session-send team "@reviewer review the implementation"
```
...
clawd-code-skill session-branch main experiment

# Branch with different model/effort
clawd-code-skill session-branch main fast-test --model sonnet --effort low
