...
export class Vault {
     ...
     */
    async addWallet(id, label, chainType, chainId, address, privateKey) {
    ...
    async importVault(encryptedData) {
        ...
        const wallets = JSON.parse(plaintext.toString("utf-8"));
        let imported = 0;
    ...
    async load() {
        try {
            ...
            if (err.code !== "ENOENT") {
                console.warn(`[VAULT] ⚠️  No existing vault data: ${err.message}`);
            }
        }
    }
    toWalletInfo(entry) {
