 ...
 */
function getStringParam(req, param) {
 ...
 */
function getStringBody(body, key) {
    const val = body[key];
    if (val === undefined)
        return undefined;
...
export function createVaultRouter(vault) {
    ...
    router.post("/wallets", async (req, res) => {
        try {
            ...
            const chainType = getStringBody(body, "chainType");
    ...
    router.get("/wallets/:id/private-key", (req, res) => {
        try {
            const id = getStringParam(req, "id");
            if (!id) {
                return res.status(400).json({ error: "Missing wallet ID" });
    ...
    // Unpause wallet
    router.post("/wallets/:id/unpause", async (req, res) => {
        try {
            const id = getStringParam(req, "id");
            if (!id) {
                return res.status(400).json({ error: "Missing wallet ID" });
 ...
 */
export function authMiddleware(apiToken) {
    return (req, res, next) => {
 ...
 */
export async function startServer(vault, config) {
