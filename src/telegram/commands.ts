 ...
 *
 * Telegram command registry for solana-clawd.
 *
 * Commands:
 ...
 *   /skills     — list solana-clawd skills
...
export async function cmdStart(ctx: CommandContext): Promise<void> {
  await ctx.reply(
    `🔱 *solana-clawd* — Autonomous Solana Trading Intelligence\n\n` +
...
export async function cmdTailscale(ctx: CommandContext): Promise<void> {
  ...
  await ctx.reply(
    ...
    `Expose solana-clawd to mobile/internet without port forwarding.\n\n` +
    `**1. Install Tailscale**\n` +
