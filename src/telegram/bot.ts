...
export class SolanaClawdBot {
  ...
  private async dispatch(msg: TgMessage): Promise<void> {
    ...
    try {
      if (
        SolanaClawdBot.privateChatOnlyCommands.has(rawCmd) &&
      ...
      if (
        SolanaClawdBot.adminOnlyCommands.has(rawCmd) &&
  ...
  async startPolling(): Promise<void> {
    ...
    console.log("🤖 solana-clawd Telegram bot started (long-polling)");
  ...
  async startWebhook(): Promise<void> {
    ...
    const secretToken = process.env.TELEGRAM_WEBHOOK_SECRET ?? "solana-clawd-secret";
    ...
    const server = http.createServer(async (req, res) => {
      if (req.method !== "POST" || req.url !== webhookPath) {
        res.writeHead(200);
        res.end("solana-clawd bot is running");
        return;
      }
    ...
    server.listen(port, () => {
      console.log(`🤖 solana-clawd bot running in webhook mode`);
...
export async function startTelegramBot(): Promise<void> {
  ...
  const bot = new SolanaClawdBot(config);
