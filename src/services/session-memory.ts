 ...
 *
 * Session Memory Service — adapted from Clawd's SessionMemory service.
 *
...
const SESSION_DIR = path.join(os.homedir(), ".config", "solana-clawd", "sessions");
...
// ─── Compact (adapted from Clawd's compact service) ────────────────────
 ...
 * Adapted from Clawd's compact service — replaces old messages with summary.
 *
 * In production, run a compact agent (like Clawd's /compact command)
...
// ─── Persistence (adapted from Clawd session storage) ──────────────────
...
// ─── Tool use summary (adapted from Clawd toolUseSummaryGenerator.ts) ──

/**
 * Generate a short summary label for a completed tool chain.
 * Adapted from Clawd's toolUseSummaryGenerator.ts (which uses claude-haiku).
...
export function generateToolChainSummary(toolChain: string[]): string {
  ...
  // Pattern-based labelling (adapted from Clawd's haiku prompt examples)
