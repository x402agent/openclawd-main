 ...
 *
 * Adapted from Clawd's services/autoDream/autoDream.ts.
 * Key changes:
...
const DREAM_LOCK_PATH = path.join(os.homedir(), ".config", "solana-clawd", "dream.lock");
...
// ─── State (closure-scoped, adapted from Clawd autoDream) ───────────────

let _dreamInProgress = false;
let _lastDreamAt = 0;
let _dreamCyclesAtLastDream = 0;

// ─── Lock (adapted from Clawd consolidationLock.ts) ────────────────────
...
// ─── Consolidation prompt (adapted from Clawd consolidationPrompt.ts) ──
 ...
 * Adapted from Clawd's executeAutoDream().
 */
