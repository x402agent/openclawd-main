 ...
 *
 * Memory extraction service — adapted from Clawd's extractMemories.ts.
 *
 ...
 *
 * Clawd uses a forked haiku agent to extract memories from transcripts.
 * solana-clawd uses a rule-based extractor tuned for Solana trading context,
...
// ─── Extractor state (closure-scoped, adapted from Clawd) ───────────────
 ...
 * Adapted from Clawd's executeExtractMemories() + runExtraction().
 */
export async function extractMemories(
  ...
  try {
    // Find messages since last extraction (adapted from Clawd cursor pattern)
    ...
    // Advance cursor (adapted from Clawd's lastMemoryMessageUuid)
