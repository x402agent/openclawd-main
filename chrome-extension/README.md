...
pAGENT injects `window.PAGENT` into every page. Your LLM client (Clawd Desktop, Cursor, Cline, any MCP client) can drive your browser with natural language.
...
- **MCP bridge** — plugs into Clawd Desktop, Cursor, Cline via stdio. One-command setup, zero browser-side config.
...
The `mcp/` package bridges pAGENT to Clawd Desktop or any MCP client:

```
Clawd Desktop / Cursor / Cline
    ↓ stdio (MCP protocol)
...
Add to your Clawd Desktop config:
