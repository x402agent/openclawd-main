...
const API_TOKEN =
  process.env.TAILCLAWD_TOKEN || process.env.TAILCLAWD_TOKEN || null;
...
async function handleHealth(
  ...
  res.end(
    JSON.stringify({
      ...
      publishedUrl: tsUrl.includes("tailclawd.local") ? null : tsUrl,
      engine: {
...
export function startProxy(): Promise<void> {
  ...
  if (!API_TOKEN) {
    logger.warn(
      "No TAILCLAWD_TOKEN or TAILCLAWD_TOKEN set — proxy is open to all tailnet peers",
    );
  }
