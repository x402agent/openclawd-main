//
//  ClawdAPI.swift
//  clawd API Implementation with streaming support
//

import Foundation

/// clawd API helper with streaming for progressive text display.
class ClawdAPI {
    ...
    /// Send a vision request to Clawd with streaming.
    ...
    func analyzeImageStreaming(
        ...
        print("🌐 clawd streaming request: \(String(format: "%.1f", payloadMB))MB, \(images.count) image(s)")
    ...
    func analyzeImage(
        ...
        request.httpBody = bodyData
        let payloadMB = Double(bodyData.count) / 1_048_576.0
        print("🌐 clawd request: \(String(format: "%.1f", payloadMB))MB, \(images.count) image(s)")
