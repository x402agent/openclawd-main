...
//  leanring-buddy
//
//  Uses clawd's Computer Use API to identify the screen location of UI elements
...
/// Detects the screen location of UI elements in screenshots using clawd's Computer Use API.
/// The Computer Use tool definition activates Clawd's specialized pixel-counting training,
...
/// Macs are 16:10 → 1280x800. This avoids distorting the image Clawd sees, which
...
class ElementLocationDetector {
    ...
    func detectElementLocation(
        ...
        // into 4:3), which would distort the image Clawd sees and degrade X-axis accuracy.
        ...
        // Clamp coordinates to the valid range — Clawd occasionally returns
    ...
    /// Calls the clawd Computer Use API with a resized screenshot and user question.
    /// Returns the raw coordinate from clawd's response in the declared resolution space, or nil.
    ...
    /// Clawd returns a `tool_use` content block with `{"action": "left_click", "coordinate": [x, y]}`.
    /// If Clawd returns text instead (no element found), returns nil.
    private func parseCoordinateFromResponse(data: Data) -> CGPoint? {
        ...
        // Look for a tool_use content block (clawd's Computer Use response format)
        ...
        // No tool_use block found — clawd responded with text (no element to point at)
    ...
    /// NSImage). This means the JPEG sent to Clawd would be 2× larger than the
    /// resolution declared in the Computer Use tool definition, causing Clawd's
