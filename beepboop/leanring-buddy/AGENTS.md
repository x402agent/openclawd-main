...
macOS menu bar companion app built with SwiftUI. Push-to-talk voice pipeline with Clawd vision, multi-screen capture, streaming AI responses, and a lobster claw overlay that points at UI elements.
...
leanring_buddyApp (entry)
    ...
    |       |
    |       +-- ClawdAPI (streaming Clawd vision)
...
- `@Published detectedElementScreenLocation` -- parsed from Clawd response, drives claw flight animation
- `@Published detectedElementDisplayFrame` -- which display the detected element is on
- Owns `BuddyDictationManager`, `ClawdAPI`, `ElevenLabsTTSClient`, `CompanionResponseOverlayManager`
- `startListening()` -- begins push-to-talk capture pipeline
- `stopListening()` -- finalizes transcript, triggers screen capture + Clawd call
- Voice flow: mic capture -> transcription -> screenshot -> Clawd vision -> TTS -> overlay
...
- Routes final transcript back to CompanionManager for Clawd API call
...
### ClawdAPI.swift
- `ClawdAPI` -- streaming Clawd API client with vision support (~291 lines)
...
### OpenAIAPI.swift
- `OpenAIAPI` -- GPT-4o alternative to Clawd (~142 lines)
...
- Parses Clawd's `[CLAW:x,y:label:screenN]` tags from response text (~335 lines)
