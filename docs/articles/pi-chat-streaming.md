...
Scope:
- Describes how Studio connects to the SolanaOS Gateway, how runtime streaming arrives over WebSockets, and how the UI renders it.
- Treats **PI** as “the coding agent running behind the Gateway” (an SolanaOS agent). Studio does not implement PI logic; it displays and controls the Gateway session.

Non-scope:
- PI internals and model/tool execution details. Those live in the SolanaOS repository and the Gateway implementation.
...
- Browser gateway protocol client (vendored): `src/lib/gateway/solanaos/GatewayBrowserClient.ts`
...
## Relationship To SolanaOS (What’s Vendored Here)

Studio vendors the browser Gateway client used to speak the Gateway protocol:
- Vendored client: `src/lib/gateway/solanaos/GatewayBrowserClient.ts`
- Sync script: `scripts/sync-solanaos-gateway-client.ts`
...
## Upstream Source Of Truth (SolanaOS)

For chat streaming behavior, these upstream files are authoritative:
- `src/gateway/protocol/schema/logs-chat.ts` in your SolanaOS checkout (`chat.send`, `chat.history`, and chat event schema)
- `src/gateway/server-methods/chat.ts` in your SolanaOS checkout (`chat.send` ack + idempotency, `chat.history` payload shaping/sanitization)
- `src/gateway/server-chat.ts` in your SolanaOS checkout (`agent` event fanout and synthetic `chat` delta/final bridging)
- `src/agents/pi-embedded-subscribe.ts` and related handlers in your SolanaOS checkout (`assistant`/`tool`/`lifecycle` stream emission)
...
- Gateway (upstream): the SolanaOS Gateway WebSocket server (default `ws://localhost:18789`).
...
sequenceDiagram
  ...
  participant G as SolanaOS Gateway (upstream)
...
- `~/.solanaos/claw3d/settings.json` (see `README.md` for the canonical location)
...
- The browser-side Gateway client (`src/lib/gateway/solanaos/GatewayBrowserClient.ts`) closes the WebSocket with close code `4008` and a reason like `connect failed: <CODE> <MESSAGE>` after it receives a failed `res(connect)`. `GatewayClient.connect()` parses that close into `GatewayResponseError(code, message)` for UI retry policy and user-facing errors.
...
- Transport reconnect (after a successful hello): the vendored browser client reconnects the browser->Studio WebSocket with backoff when it closes, and continues emitting events after reconnect. See `src/lib/gateway/solanaos/GatewayBrowserClient.ts`.
...
Files:
- Gap detection: `src/lib/gateway/solanaos/GatewayBrowserClient.ts`
...
- `MEDIA: /home/ubuntu/.solanaos/.../image.png`
...
3. The API route reads the image either locally (only under `~/.solanaos`) or over SSH for remote gateways, and returns the bytes with the correct `Content-Type`.

Files:
- Rewrite helper: `src/lib/text/media-markdown.ts`
- Media API route: `src/app/api/gateway/media/route.ts`
- SSH helper + env vars (`SOLANAOS_GATEWAY_SSH_TARGET`, `SOLANAOS_GATEWAY_SSH_USER`): `src/lib/ssh/gateway-host.ts`
