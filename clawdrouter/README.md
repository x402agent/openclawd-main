...
> **Prefer hosted?** The same router runs at [`https://clawdrouter.x402.workers.dev`](https://clawdrouter.x402.workers.dev) on Cloudflare Workers — skip steps 1–2 and mint a wallet-signed API key on [solanaclawd.com/api](https://solanaclawd.com/api). See [Hosted edge](#hosted-edge) below.
...
The same router also runs on Cloudflare Workers at `https://clawdrouter.x402.workers.dev` — no install required.

| Surface | What it gives you |
| --- | --- |
| `POST /v1/chat/completions` | OpenAI-compatible endpoint with the 15-dim scorer + profile routing running at the edge. |
| `GET /v1/stats` | Realtime snapshot — totals + last 50 routes — served by a global Durable Object. |
| `GET /v1/stats/stream` | WebSocket feed. Every routed request pushes a `{type:"route"}` frame. |
| `GET /v1/stats/history` | Historical aggregations pulled from Cloudflare Analytics Engine (`dataset: clawd`). |
| `POST /v1/keys/{challenge,mint,revoke}` · `GET /v1/keys` | Mint wallet-signed API keys. Sign a challenge with your Solana wallet; the Worker returns a `ck_live_…` key stored as SHA-256 in D1. |

```bash
# Mint a key via solanaclawd.com/api (Phantom → sign → copy key).
# Then use it like any OpenAI endpoint:
curl -X POST https://clawdrouter.x402.workers.dev/v1/chat/completions \
  -H "authorization: Bearer ck_live_..." \
  -H "content-type: application/json" \
  -d '{"model":"clawdrouter/auto","messages":[{"role":"user","content":"hi"}]}'
```

Each request writes a typed datapoint to the `clawd` Analytics Engine dataset (model / tier / provider / tokens / cost / savings / latency / scorer score) AND pushes a live event to the `ClawdStats` Durable Object.

**Full architecture:** [`docs/clawdrouter-cloud.md`](../docs/clawdrouter-cloud.md). **Site surface:** [`/api`](https://solanaclawd.com/api), [`/stats`](https://solanaclawd.com/stats), [`/latest`](https://solanaclawd.com/latest).
