...
import { PRIORITY_FEE, SLIPPAGE } from '../constants';

export const getBuyTxWithJupiter = async (wallet: Keypair, baseMint: PublicKey, amount: number) => {
  try {
    const quoteResponse = await (
      await fetch(
        `https://quote-api.jup.ag/v6/quote?inputMint=So11111111111111111111111111111111111111112&outputMint=${baseMint.toBase58()}&amount=${amount * LAMPORTS_PER_SOL}&slippageBps=${SLIPPAGE}`
      )
    ).json();
    ...
    const { swapTransaction } = await (
      await fetch("https://quote-api.jup.ag/v6/swap", {
        ...
        body: JSON.stringify({
          ...
          prioritizationFeeLamports: PRIORITY_FEE * LAMPORTS_PER_SOL
        }),
      })
    ).json();
...
export const getBuyInStructionsWithJupiter = async (wallet: Keypair, baseMint: PublicKey, amount: number) => {
  try {
    const quoteResponse = await (
      await fetch(
        `https://quote-api.jup.ag/v6/quote?inputMint=So11111111111111111111111111111111111111112&outputMint=${baseMint.toBase58()}&amount=${amount * LAMPORTS_PER_SOL}&slippageBps=${SLIPPAGE}`
    ...
    // get serialized transactions for the swap
    const { setupInstructions, addressLookupTableAddresses } = await (
