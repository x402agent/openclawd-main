...
> Visualize AI coding sessions in Claw3D's retro office — without SolanaOS.
...
Agent Bus includes an **SolanaOS-compatible gateway** that speaks the same WebSocket protocol Claw3D already uses. No Claw3D code changes needed — just point `GATEWAY_URL` to the Agent Bus gateway.
...
│ SolanaOS protocol v2                     │
│ In-memory agent registry                 │
│ 10 RPC methods (connect, agents.list...) │
└────────────────────┬─────────────────────┘
                     │ WebSocket (SolanaOS frames)
                     ▼
┌──────────────────────────────────────────┐
│ Claw3D (:3000)                           │
...
- Gateway on `:18789` (SolanaOS protocol)
...
Restart Claw3D. It will connect to the Agent Bus gateway instead of SolanaOS.
...
The Agent Bus gateway implements SolanaOS protocol v2:
...
## Key Differences from SolanaOS

| Feature | SolanaOS | Agent Bus |
|---------|----------|-----------|
...
| Agent sources | SolanaOS agents only | Any (Claude Code, Gemini, cron, etc.) |
