...
- How “create agent” choices in **Claw3D** flow into the **SolanaOS Gateway** (often running on an EC2 host) where enforcement actually happens

Scope:
- Studio one-step agent creation and post-create authority updates, including exact gateway calls.
- The upstream SolanaOS implementation that persists and enforces those settings at runtime.
...
1. Writes **configuration** into the Gateway (per-agent overrides in `solanaos.json`).
2. Writes **policy** into the Gateway (per-agent exec approvals in `exec-approvals.json`).

The Gateway (SolanaOS) is the enforcement point:
...
- **Gateway**: SolanaOS Gateway WebSocket server (upstream project).
- **Studio**: this repo. Next.js UI plus a Node WS proxy.
- **Agent**: an SolanaOS agent entry stored in gateway config (`agents.list[]`).
- **Session key**: SolanaOS session identifier. Studio uses `agent:<agentId>:<mainKey>` for the agent’s “main” session.
...
- **Tool policy** (`tools.profile`, `tools.alsoAllow`, `tools.deny`): allow/deny gating for PI tools (SolanaOS resolves effective policy).
...
Studio capability updates rely on SolanaOS tool-group expansion (`solanaos/src/agents/tool-policy.ts`), especially:
...
sequenceDiagram
  ...
  participant G as SolanaOS Gateway

  UI->>L: submit({ name, avatarSeed? })
  L->>GC: createGatewayAgent(name)
  GC->>G: config.get
  G-->>GC: { path: ".../solanaos.json", ... }
...
## SolanaOS (Upstream): What `agents.create` Actually Does

Gateway method:
- `solanaos/src/gateway/server-methods/agents.ts` (`"agents.create"`)
...
## SolanaOS (Upstream): Sandbox Semantics

Sandbox configuration resolution:
- `solanaos/src/agents/sandbox/config.ts` (`resolveSandboxConfigForAgent`)

Sandbox context creation (where workspace selection happens):
- `solanaos/src/agents/sandbox/context.ts` (`resolveSandboxContext`)

Docker mount behavior:
- `solanaos/src/agents/sandbox/docker.ts` (`createSandboxContainer`)
...
- `solanaos/src/agents/sandbox/runtime-status.ts` (`resolveSandboxRuntimeStatus`)
...
Upstream implementation reference:
- `solanaos/src/agents/sandbox/types.ts` (`SandboxScope`)
- `solanaos/src/agents/sandbox/shared.ts` (`resolveSandboxScopeKey`)
...
- `solanaos/src/agents/sandbox/constants.ts` uses `<STATE_DIR>/sandboxes` (where `STATE_DIR` defaults to `~/.solanaos` unless overridden by `SOLANAOS_STATE_DIR`).
...
- When using a sandbox workspace root, upstream seeds missing bootstrap files from the agent workspace and ensures bootstrap exists:
  - `solanaos/src/agents/sandbox/workspace.ts` (`ensureSandboxWorkspace`)
...
In upstream SolanaOS, sandboxed filesystem tools are rooted and guarded:
- `solanaos/src/agents/pi-tools.read.ts` (`assertSandboxPath` usage)
...
SolanaOS has an additional sandbox-only tool allow/deny policy:
- `tools.sandbox.tools.allow|deny` (global)
- `agents.list[].tools.sandbox.tools.allow|deny` (per-agent override)

Upstream resolution:
- `solanaos/src/agents/sandbox/tool-policy.ts` (`resolveSandboxToolPolicyForAgent`)
...
## SolanaOS (Upstream): Tool Availability and `workspaceAccess=ro`

PI tool construction:
- `solanaos/src/agents/pi-tools.ts` (`createSolanaOSCodingTools`)
...
Separately from per-agent config and exec approvals, SolanaOS supports per-session exec settings:
...
- Upstream method: `solanaos/src/gateway/server-methods/sessions.ts` (`"sessions.patch"`)
- Patch application: `solanaos/src/gateway/sessions-patch.ts`
- Session entry shape includes `execHost|execSecurity|execAsk`: `solanaos/src/config/sessions/types.ts`
...
## SolanaOS (Upstream): Exec Approvals (Policy + Events)

Exec approvals file (defaults upstream):
- `solanaos/src/infra/exec-approvals.ts`
  - default file path: `~/.solanaos/exec-approvals.json`
  - default socket path: `~/.solanaos/exec-approvals.sock`

Gateway methods (persist policy):
- `solanaos/src/gateway/server-methods/exec-approvals.ts`
...
- `solanaos/src/gateway/server-methods/exec-approval.ts`
  - broadcasts `exec.approval.requested`
  - broadcasts `exec.approval.resolved`

Exec tool approval decision logic:
- `solanaos/src/agents/bash-tools.exec.ts` (calls `requiresExecApproval`, `evaluateShellAllowlist`, etc.)
...
1. Determine if the session is sandboxed and what workspace it is using.
   - Upstream CLI helper: `solanaos sandbox explain --agent <agentId>` (see upstream `src/commands/sandbox-explain.ts`)
