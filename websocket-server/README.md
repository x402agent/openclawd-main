# CLAWD WebSocket Relay Server

Real-time Solana market-data relay for the site. It streams Pump launches over
WebSocket, exposes REST proxies for BirdEye / Solana Tracker / Jupiter / dFlow,
and accepts enhanced Helius webhooks at `/webhook/helius` so the frontend can
consume one live backend instead of talking to each vendor separately.
...
The server starts on `http://localhost:3099` and serves a built-in dashboard UI
at the root path.

Production Fly deployment:

- HTTP: `https://clawd-ws.fly.dev`
- WebSocket: `wss://clawd-ws.fly.dev/ws`
- Helius webhook: `https://clawd-ws.fly.dev/webhook/helius`
...
| `POST` | `/webhook/helius` | Enhanced Helius webhook ingress |
| `GET` | `/api/birdeye/*` | BirdEye proxy |
| `GET` | `/api/tracker/*` | Solana Tracker proxy |
| `GET` | `/api/jupiter/*` | Jupiter price / quote / prediction proxy |
| `GET` | `/api/dflow/*` | dFlow quote / prediction proxy |
...
| `HELIUS_RPC_URL` | — | Preferred Solana HTTP RPC URL |
| `HELIUS_WSS_URL` | — | Preferred Solana WebSocket RPC URL |
| `HELIUS_API_KEY` | — | Enables Parse API fallback + Helius status |
| `HELIUS_WEBHOOK_AUTH` | — | Shared secret for `/webhook/helius` |
...
HELIUS_API_KEY=... \
HELIUS_WEBHOOK_AUTH=clawd_wh_replace_me \
npm start
```
