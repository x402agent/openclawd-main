...
const MAX_QUEUE = parseInt(process.env.CLAIM_QUEUE_MAX || '50', 10);
const MIN_INTERVAL_MS = parseInt(process.env.CLAIM_QUEUE_INTERVAL_MS || '1000', 10);
const MAX_BACKOFF_MS = 60_000;
const RATE_LIMIT_LOG_WINDOW_MS = 30_000;

function is429(err: unknown): boolean {
  const msg = err instanceof Error ? err.message : String(err);
  return msg.includes('429') || msg.includes('Too Many Requests') || msg.includes('Too many requests');
}
...
class TxQueue {
  ...
  // adaptive backoff
  private backoffMs = 0;
  // suppress repeat 429 log spam
  private last429LogTime = 0;
  private suppressed429Count = 0;
  ...
  private async drain(): Promise<void> {
    ...
    while (this.queue.length > 0) {
      const interval = Math.max(MIN_INTERVAL_MS, this.backoffMs);
      ...
      try {
        ...
        if (is429(e)) {
          // Exponential backoff on 429, capped
          this.backoffMs = this.backoffMs === 0
            ? 2_000
            : Math.min(MAX_BACKOFF_MS, this.backoffMs * 2);
          // Drop queued work — no point piling up while we're throttled
          const dropped = this.queue.length;
          this.queue = [];
          // Suppress repeat log spam
          this.suppressed429Count++;
          const now = Date.now();
          if (now - this.last429LogTime > RATE_LIMIT_LOG_WINDOW_MS) {
            console.warn(
              `[claims] RPC 429 rate-limited — backing off ${this.backoffMs}ms ` +
              `(${this.suppressed429Count} hits in last ${Math.round((now - this.last429LogTime) / 1000)}s, ${dropped} dropped). ` +
              `Set SOLANA_RPC_URL to a paid RPC (Helius, QuickNode, Triton) for reliable claim detection.`,
            );
            this.last429LogTime = now;
            this.suppressed429Count = 0;
          }
        } else {
