...
The easiest way to run chess-mcp with [Clawd Desktop](https://clawd.ai/desktop) is using Docker. If you don't have Docker installed, you can get it from [Docker's official website](https://www.docker.com/get-started/).


Edit your Clawd Desktop config file:
* Mac: `~/Library/Application Support/Clawd/clawd_desktop_config.json`
* Windows: `%APPDATA%/Clawd/clawd_desktop_config.json`
* Linux: `~/.config/Clawd/clawd_desktop_config.json`
...
Alternatively, you can run the server directly using UV. Edit your Clawd Desktop config file (locations listed above) and add the server configuration:
...
> Note: if you see `Error: spawn uv ENOENT` in [Clawd Desktop](https://clawd.ai/desktop), you may need to specify the full path to `uv` or set the environment variable `NO_UV=1` in the configuration.
